...
TEXT_SUFFIXES = {
    ...
    ".sh", ".txt", ".yml", ".yaml", ".plist", ".conf", ".ini",
}

ROOT_ALLOW_FILES = {
    ".gitignore", "AGENT.md", "AGENTS.md", "README.md", "metadata.json",
}

ROOT_ALLOW_DIRS = {
    ".github", ".githooks", "docs", "mac", "skills", "surface",
}

RUNTIME_BASENAMES = {
    "event_stream.json", "local_event_search_results.json", "market.json",
    "market_config.json", "metrics.json", "photos.json", "schedule.json",
    "sync_status.json", "weather.json",
}
...
OBSOLETE_LOCAL_EVENT_NAMES = (
    "local_events_controls", "local_events_compact", "local_events_carousel",
...
SECRET_RE = re.compile(
    ...
    r"glpat-[A-Za-z0-9_-]{20,}|AKIA[A-Z0-9]{16}|"
    r"sk-[A-Za-z0-9]{16,})\b"
)
...
def changed_paths(base, head, include_working_tree):
    names = set(
        split_null_output(
            git_output(["diff", "--name-only", "-z", f"{base}...{head}"])
        )
    )
    if include_working_tree:
        names.update(
            split_null_output(git_output(["diff", "--name-only", "-z"]))
        )
        names.update(
            split_null_output(git_output(["diff", "--cached", "--name-only", "-z"]))
        )
        names.update(
            split_null_output(
...
def is_text_file(path):
    return (
        ...
        or path.name in {"README", "README.md", ".gitignore"}
    )
...
def check_path_policy(relative, errors):
    ...
    if len(parts) == 1:
        if path.name not in ROOT_ALLOW_FILES:
            add_error(
                errors,
                relative,
                "root file is outside the repository root allowlist",
            )
        if suffix in {".css", ".js"}:
            add_error(
                errors,
                relative,
                "root browser asset is not an active source path",
            )
    elif parts[0] not in ROOT_ALLOW_DIRS:
        add_error(
            errors,
            relative,
            "top-level directory is outside the repository root allowlist",
        )
    ...
    if (
        ...
        and parts[0] == "surface"
        and parts[1] == "web"
        and suffix in {".css", ".js"}
    ):
        add_error(
            errors,
            relative,
            "legacy direct surface/web browser asset; use surface/web/assets",
        )
    ...
    if any(normalized.startswith(prefix) for prefix in RUNTIME_PREFIXES):
        add_error(
            errors,
            relative,
            "runtime/generated/local artifact path must not be committed",
        )
    ...
    if any(part in RUNTIME_PARTS for part in parts):
        add_error(
            errors,
            relative,
            "cache path must not be committed",
        )
    ...
    if name in RUNTIME_BASENAMES and not under_test_fixture(normalized):
        add_error(
            errors,
            relative,
            "runtime/generated JSON must not be committed outside fixtures",
        )

    if (
        suffix in {".pyc", ".tmp", ".log", ".bak", ".backup"}
        or ".bak" in name
        or name.endswith(".back")
    ):
        add_error(
            errors,
            relative,
            "generated, cache, log, or backup file must not be committed",
        )
...
def main():
    ...
    print(
        f"PASSED suite={args.suite} scope={args.scope} files={len(paths)}"
    )
    return 0
