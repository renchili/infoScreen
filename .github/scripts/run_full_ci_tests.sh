...
log_step seed_runtime_data python3 -c 'import os, shutil; from pathlib import Path; root=Path.cwd(); out=Path(os.environ["INFOSCREEN_ENV_DIR"]); [shutil.copy2(p, out/p.name) for p in (root/"surface/tests/fixtures/runtime_data").glob("*.json")]; (out/"public_photos"/"fixture-photo.txt").write_text("fixture photo bytes\n", encoding="utf-8")'
log_step repo_hygiene python3 .github/scripts/check_repo.py --suite all --scope repository
log_step metadata_json python3 -m json.tool metadata.json
log_step python_compile python3 -m compileall -q surface mac .github/scripts .github/tests
...
log_step shell_syntax bash -n .github/scripts/run_full_ci_tests.sh
log_step pytest python3 -m pytest -c .github/pytest.ini --junitxml "$ARTIFACT_DIR/pytest-junit.xml"
