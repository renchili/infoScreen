...
from local_events_runtime.structural_link_authority import (  # noqa: E402
    _configured_selectors,
    apply as apply_structural_link_authority,
)
...
def test_calendar_of_events_uses_verified_activity_card_boundary() -> None:
    ...
    assert selectors["gardensbythebay"] == [
        "a.programme-title.row-listing-title[href]"
    ]
    ...
    assert "repeatedCardAnchors.length" in diagnostic_js
    assert "routeDetailAnchors" in diagnostic_js
