...
def test_studio_preview_uses_isolated_route_without_writing_decisions() -> None:
    ...
    assert "body: JSON.stringify({ listing_url: url })" in preview
    assert "Confirm this list page before previewing it" not in preview
    assert "REVIEW DECISION UNCHANGED" in preview
