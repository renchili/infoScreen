...
def test_preview_listing_and_details_borrow_one_request_local_browser(
    ...
    monkeypatch.setattr(authority._review, "launch_chromium", authority.launch_or_borrow)
    ...
    def base_collect(store):
        # The real listing owner uses event_review.launch_chromium, while detail
        # enrichment uses browser.launch_chromium. Both must resolve to one lease.
        from playwright.sync_api import sync_playwright

        with sync_playwright() as listing_playwright:
            listing_browser = authority._review.launch_chromium(listing_playwright)
...
def test_reapply_restores_both_listing_and_detail_lease_launchers(monkeypatch) -> None:
    ...
    monkeypatch.setattr(authority._review, "launch_chromium", replacement)

    authority.apply()

    assert authority._BASE_BROWSER_LAUNCH is fallback
    assert authority._browser.launch_chromium is authority.launch_or_borrow
    assert authority._review.launch_chromium is authority.launch_or_borrow
