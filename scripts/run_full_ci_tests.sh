...
log_step static_inventory python3 scripts/static_inventory.py
log_step metadata_json python3 -m json.tool metadata.json
log_step python_compile python3 -m compileall -q surface tests scripts/static_inventory.py
...
python3 -c 'import json, os, subprocess; from pathlib import Path; d=Path(os.environ["ARTIFACT_DIR"]); summary=d/"summary.md"; report={"commit": subprocess.check_output(["git","rev-parse","HEAD"], text=True).strip(), "artifact_dir": str(d), "runtime_env": os.environ["INFOSCREEN_ENV_DIR"], "logs": sorted(str(p) for p in d.glob("*.log")), "junit": str(d/"pytest-junit.xml"), "inventory": str(d/"static-inventory.json"), "summary": str(summary), "summary_text": summary.read_text(encoding="utf-8")}; (d/"report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")'
