...
## 官方页面已经提供结构化活动数据时应该怎么抓取？

决定：所有来源都先尝试读取官方页面加载的 XHR/fetch JSON、JSON-LD、`application/json` 脚本和常见页面状态对象，再使用该来源原有的 DOM 或详情页 adapter 补充没有结构化数据的活动。结构化数据和 DOM 中标题相同的记录以结构化数据为准。

原因：不能假设某一个机构一定有可读取的网络 JSON，也不能把结构化能力只绑定到单个机构。把结构化读取作为所有来源的第一层，并保留原有 adapter 作为同一页面会话内的回退，既能直接使用独立的开始和结束日期字段，也不会因为某个站点没有 JSON 就丢失全部活动。

## 活动字段在当前页面不完整时应该怎么处理？

决定：不能因为当前节点或当前页面缺少某个字段就丢弃整条活动。采集器必须继续沿同一活动的可验证关联读取列表卡、详情页、官方接口或嵌入式状态，并通过稳定的活动 URL、官方 ID 或明确的一对一关系合并字段。确实在官方来源中不存在的字段可以保留为空，同时记录字段来源和缺失状态；不得用其他活动的内容、机构默认文案或推测值冒充真实字段。

原因：字段缺失首先是采集流程尚未完成，不是拒绝输出的理由。正确做法是完成同一活动的证据链并保留可追溯来源，而不是把解析失败转化为活动丢失，也不是用无法验证的兜底值掩盖问题。
