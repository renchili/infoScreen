...
决定：优先读取页面加载时返回的官方 XHR/fetch JSON，直接映射标题、开始日期、结束日期、场地和官方链接；只有没有结构化数据的来源才使用 DOM adapter。

原因：开始日期和结束日期是独立字段时，不应该再从展示文本中猜测日期范围。直接读取官方结构化数据可以避免星期、换行、样式节点和隐藏文本改变后造成日期被截断。
