...
The Studio must require a decision for every returned Preview candidate. It stores the active URL, parser-versioned structured candidate rows, original/final detail identities, summaries, diagnostics, and draft decisions in `sessionStorage`, never HTML. Initial state loading, manual reload, List Page decision rendering, and returning from another browser tab must reconstruct the active panel and reapply drafts when the List Page and parser version still match. Formal Event collection clears the active temporary panel; closing the tab clears uncommitted browser drafts.

The Studio sends the complete set to `POST /api/local-events/review/listing-decision` in the `preview-review-v1:` envelope, including the original candidate identity, original `listing_detail_url`, final redirected/public `detail_url`, and REAL EVENT / NOT EVENT decision. The backend compares the submitted set with the latest eligible manifest, stores the reviewed set in `preview_event_selections.json`, writes the List Page decision, and restores the prior selection file when the state write raises. A failed state write keeps the manifest available for retry. Committed selection state remains authoritative across browser rendering and tab changes.
...
Classify every candidate, switch browser tabs and return, then save the List Page review: the active Preview panel and classifications must survive the return-state render, and the committed selection set and List Page decision must agree. Reopening the Studio in the same tab session must reconstruct the version-compatible structured Preview without restoring HTML. Confirming requires at least one REAL EVENT; rejecting forbids a REAL EVENT. Formal collection must open and persist only selected REAL EVENT candidates, clear the temporary active panel, and include a selected candidate whose original list link redirects to a different final public URL. A legacy confirmed archive page with an old committed selection must not be admitted into formal collection.
