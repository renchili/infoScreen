...
A correct listing card may omit date and venue. After admission, the collector follows only that card’s official detail URL. Within that detail document, values attached to explicit activity labels such as `Opening Hours`, `Date & Time`, `Date`, `Time`, and `Location`, or to a structured Event whose identity matches the current activity heading, retain separate provenance through final field selection. Nested heading markup is read by its complete visible label text, and the primary activity heading may use any semantic heading level. Generic date/location selectors, navigation labels, promotions, announcements, and document facts remain untrusted evidence and are never rewritten into synthetic `Date` or `Location` sections. A provenance-aware detail is marked `collected` only when trusted schedule and venue fields are present; otherwise it remains `incomplete` with the missing-field error. Detail failure does not erase the list evidence; review candidates remain visible with exact detail status/error.
...
The browser keeps the active Preview URL, parser-versioned structured candidate rows, original/final detail identities, summaries, diagnostics, and uncommitted REAL EVENT / NOT EVENT choices in `sessionStorage`. It never stores or restores candidate-panel HTML. Initial state loading, manual `RELOAD`, saving a List Page review, and returning from another browser tab may rebuild the server-owned Studio DOM; after that render, the Preview owner restores the active panel only when the List Page still exists and the stored `detail_field_authority_version` matches the running parser, then reapplies the draft choices. Closing the browser tab removes these temporary browser drafts, while a completed formal Event collection explicitly clears the active Preview because persisted Event candidates supersede it.

The structured browser snapshot is display and draft-continuity state, not server candidate-set authority. Saving still requires the latest unexpired process-local manifest, so a service restart, manifest expiry, newer Preview, or List Page revision change rejects stale submission even if the browser can still display its last structured draft. The final Preview handoff first invalidates any older manifest, completes detail enrichment and redirect handling, then issues the exact candidate manifest returned to the browser. When the operator saves the List Page review, `POST /api/local-events/review/listing-decision` receives a `preview-review-v1:` payload containing every Preview candidate and its REAL EVENT / NOT EVENT decision. The backend validates the List Page identity, official-domain detail URLs, candidate identities, and exact equality with the latest unexpired server manifest. The backend atomically replaces `preview_event_selections.json`, then writes the List Page decision; if that state write fails, the prior selection file is restored and the still-valid manifest remains available for retry. Successfully committed REAL EVENT / NOT EVENT decisions remain authoritative in `preview_event_selections.json` across state refreshes and browser-tab changes.
