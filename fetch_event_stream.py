...
def fetch(url: str, timeout: int = 18) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Surface-Info-TTY/1.0",
...
def mostly_latin(s: str) -> bool:
    letters = re.findall(r"[A-Za-zÀ-ÿ]", s or "")
...
def translate_strict(text: str, target_lang: str) -> str:
    ...
    for _ in range(3):
        try:
            out = translate_google(text, target_lang)
            if not out:
                raise RuntimeError("empty translation")

            if target_lang == "zh" and not has_cjk(out):
                raise RuntimeError("zh translation has no CJK")

            if target_lang in ("en", "fr") and not mostly_latin(out):
...
def main():
    ...
    for feed in FEEDS:
        try:
            raw = fetch(feed["url"])
            pool.extend(parse_feed(feed, raw))
        except Exception as e:
            errors.append({
                "source": feed["source"],
                "lang": feed["lang"],
    ...
    for base in pool:
        ...
        try:
            ...
            errors.append({
                "source": base.get("base_source", ""),
                "lang": base.get("base_lang", ""),
                "title": base.get("base_title", "")[:120],
                "error": str(e)[:180],
            })
            continue
    ...
    out = {
        ...
        "errors": errors,
        "selection": {
            "mode": "same random base news items, strict EN/FR/ZH translation, failed triples skipped",
