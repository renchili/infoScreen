    ...
    manual_listing_schema = {
        ...
        "properties": {
            ...
            "url": {
                ...
                "format": "uri",
                "description": (
                    "Current official HTTP/HTTPS Event listing page on the institution "
                    "allow-list. Archive and past-activities pages are rejected."
                ),
            },
        },
    }
    ...
    return {
        ...
        "paths": {
            ...
            "/api/local-events/review/preview-events": {
                "post": {"tags": ["local-event-review"], "summary": "Preview Event candidates from one saved list page", "description": "Accepts a pending, confirmed, or rejected saved list page and copies Review state into an isolated temporary store. One Preview request owns one Playwright manager with a source-specific Chromium lifecycle. Most sources reuse one process/context for the listing and details and report preview_browser_process_count=1, preview_browser_reuse=listing_and_details, preview_detail_context_count=1, and preview_detail_transport=same_browser_context. ArtScience Museum / Marina Bay Sands closes the listing browser and opens each admitted detail as the first document in a fresh sequential Chromium process/context, reporting preview_detail_fresh_browser_count, preview_browser_process_count=1+preview_detail_fresh_browser_count, preview_browser_reuse=single_playwright_sequential_browsers, preview_detail_context_count=preview_detail_fresh_browser_count, and preview_detail_transport=sequential_browser_processes. This avoids reusing the listing process network/HTTP2 connection state. Preview transport may use headed Chromium for Marina Bay Sands and records NetLog diagnostics; isolated Preview does not force HTTP/1. Ended official candidates remain visible only in Preview with preview_expiry_policy=retain_for_operator_review, while formal collection and kiosk publication keep normal expiry filtering. If listing-only detail evidence remains, the final handoff fails without issuing a manifest or opening a post-collector fallback browser. The final redirected candidate set is recorded in a process-local manifest with a default 21,600-second lifetime, a 60-second minimum, and INFOSCREEN_PREVIEW_MANIFEST_TTL_SECONDS configuration. Preview does not change persisted Review state. Candidate choices remain browser-session drafts until a separate List Page decision request commits them; a draft whose manifest is no longer valid must be Previewed again.", "requestBody": request_body(preview_listing_schema), "responses": {"200": json_response(object_schema), "400": json_response(ref("ErrorResponse"), "Missing or unknown listing URL"), "500": json_response(ref("ErrorResponse"), "Preview collection, browser lifecycle, or final-detail invariant failed")}},
            },
            "/api/local-events/review/collect-events": {
                "post": {"tags": ["local-event-review"], "summary": "Collect selected REAL EVENT candidates from confirmed pages", "description": "Reads current confirmed List Pages that have committed REAL EVENT selections. Archive and past-activities pages are excluded again at the formal selection boundary even if legacy Review state still contains them. The collector filters unselected listing cards before detail navigation, follows official detail links with the formal HTTP/1 Chromium policy, writes per-listing recognition diagnostics, and persists only selected Event candidates as confirmed. It does not change List Page decisions.", "responses": {"200": json_response(object_schema), "500": json_response(ref("ErrorResponse"), "Collection failed")}},
            },
