    ...
    return {
        ...
        "paths": {
            ...
            "/api/local-events/review/preview-events": {
                "post": {"tags": ["local-event-review"], "summary": "Preview Event candidates from one saved list page", "description": "Accepts a pending, confirmed, or rejected saved list page. The server copies Review state into an isolated temporary store and launches one request-local Chromium process with --disable-http2. Listing collection, detail enrichment, and the final listing-only fallback borrow that same browser lease; the real browser is closed exactly once. If listing-only detail evidence remains after the lease, Preview fails instead of launching a second browser. A successful response records preview_browser_process_count=1, preview_browser_reuse=listing_and_details, and preview_detail_transport=single_http1_browser_process, completes redirect/detail enrichment, then records the exact returned candidate set in a process-local manifest. The manifest defaults to a 21,600-second lifetime with a 60-second minimum and is controlled by INFOSCREEN_PREVIEW_MANIFEST_TTL_SECONDS. Preview does not change persisted Review state. Candidate choices remain browser-session drafts until a separate List Page decision request commits them; a draft whose manifest is no longer valid must be Previewed again.", "requestBody": request_body(preview_listing_schema), "responses": {"200": json_response(object_schema), "400": json_response(ref("ErrorResponse"), "Missing or unknown listing URL"), "500": json_response(ref("ErrorResponse"), "Preview collection, browser-session, or final-detail invariant failed")}},
            },
