    ...
    return {
        ...
        "paths": {
            ...
            "/api/local-events/review/listing-page": {
                "post": {"tags": ["local-event-review"], "summary": "Add one operator-supplied official Event list page", "description": "Validates the selected institution and allowed official domain, saves the page as pending review state, and does not collect Events automatically. The saved page may be previewed before a decision; normal persisted collection still uses confirmed pages only.", "requestBody": request_body(manual_listing_schema), "responses": {"200": json_response(object_schema), "400": json_response(ref("ErrorResponse"), "Invalid institution or listing URL")}},
            },
            ...
            "/api/local-events/review/preview-events": {
                "post": {"tags": ["local-event-review"], "summary": "Preview Event candidates from one saved list page", "description": "Accepts a pending, confirmed, or rejected saved list page. The server copies Review state into an isolated temporary store, marks only that copy confirmed, clears copied Events and feedback, runs the final collector, and returns the preview without changing persisted Review state.", "requestBody": request_body(preview_listing_schema), "responses": {"200": json_response(object_schema), "400": json_response(ref("ErrorResponse"), "Missing or unknown listing URL"), "500": json_response(ref("ErrorResponse"), "Preview collection failed")}},
            },
