(function () {
  ...
  function item(row, i) { return { title: val(row, ["title", "what_text", "name", "event", "summary"]), when: val(row, ["when", "when_text", "date", "date_text", "time", "time_text"]), where: val(row, ["where", "venue", "place", "where_text", "location"]), source: val(row, ["source_name", "host", "organizer", "source"]), summary: val(row, ["summary", "description", "desc", "why_text"]), order: Number(row && (row.source_order == null ? i : row.source_order)) }; }
  ...
  function draw() { var box = el("localEventList"); if (!box) return; if (!list.length) return drawEmpty("NO RENDERABLE EVENTS"); if (page < 0) page = list.length - 1; if (page >= list.length) page = 0; var x = list[page]; box.innerHTML = '<div class="local-event-card active"><div class="local-event-source">' + enc(x.source || "Official source") + '</div><div class="local-event-title">' + enc(x.title || "Local event") + '</div><div class="local-event-facts">' + fact("WHEN", x.when) + fact("WHERE", x.where) + '</div><div class="local-event-desc"><div class="local-event-desc-text">' + enc(x.summary || "") + '</div></div><div class="local-event-actions"><span class="local-event-no-link">OFFICIAL SOURCE</span></div></div>'; var c = el("localEventCounter"); if (c) c.textContent = (page + 1) + "/" + list.length; }
